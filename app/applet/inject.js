...
const rawContent = fs.readFileSync('src/data/mockData.ts', 'utf8');
...
let parts = rawContent.split('export const bestOfArticles');

if (parts.length > 1) {
  let topPart = parts[0].trim();
  if (topPart.endsWith('];')) {
    topPart = topPart.substring(0, topPart.length - 2);
  }
  let finalContent = topPart + ',\\n' + reviewString + '\\n];\\n\\nexport const bestOfArticles' + parts[1];
  fs.writeFileSync('src/data/mockData.ts', finalContent);
  console.log("SUCCESS!");
} else {
  console.log("FAILED STR SPLIT");
}
