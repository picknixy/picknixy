    ...
    <>
      ...
      <section className="relative h-[75vh] md:h-[85vh] w-full bg-black overflow-hidden flex flex-col items-center">
        ...
        <div className="relative z-10 mt-16 md:mt-24 px-4 text-center text-white">
          <h1 className="text-5xl md:text-7xl font-semibold tracking-tight mb-2 drop-shadow-lg">
            {featuredReview.title}
          </h1>
          <p className="text-xl md:text-3xl font-normal tracking-tight mb-4 max-w-2xl mx-auto drop-shadow-md">
            {featuredReview.excerpt}
          </p>
          <div className="flex justify-center gap-6 text-lg md:text-xl font-medium mt-4">
            <Link
              to={`/${featuredReview.slug}`}
              className="text-[#2997ff] hover:underline flex items-center"
            >
      ...
      <section className="relative h-[75vh] md:h-[85vh] w-full bg-[#f5f5f7] overflow-hidden flex flex-col items-center mt-3">
        ...
        <div className="relative z-10 mt-16 md:mt-24 px-4 text-center text-white">
          <h2 className="text-5xl md:text-7xl font-semibold tracking-tight mb-2 drop-shadow-lg">
            {secondReview.title}
          </h2>
          <p className="text-xl md:text-3xl font-normal tracking-tight mb-4 max-w-2xl mx-auto drop-shadow-md">
            {secondReview.excerpt}
          </p>
          <div className="flex justify-center gap-6 text-lg md:text-xl font-medium mt-4">
            <Link
              to={`/${secondReview.slug}`}
              className="text-[#2997ff] hover:underline flex items-center"
            >
              Read review <span className="ml-1 text-sm">›</span>
            </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {gridItems.map((item, idx) => {
            ...
            return (
              <div
                key={item.id}
                className="relative h-[500px] md:h-[600px] bg-black overflow-hidden flex flex-col items-center group"
              >
                <div className="absolute inset-0">
                  <img
                ...
                <div className="relative z-10 mt-12 px-8 text-center text-white w-full">
                  <h3 className="text-3xl md:text-4xl font-semibold tracking-tight mb-2 drop-shadow-lg">
                    {item.title}
                  </h3>
                  <p className="text-lg md:text-xl font-normal tracking-tight mb-4 max-w-md mx-auto line-clamp-2 drop-shadow-md">
                    {item.excerpt}
                  </p>
                  <div className="flex justify-center gap-6 text-lg font-medium mt-2">
                    <Link
                      ...
                      className="text-[#2997ff] hover:underline flex items-center"
                    >
                      {isReview ? "Read review" : "Read guide"}{" "}
