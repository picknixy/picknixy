...
export function Sidebar() {
  ...
  return (
    <aside className="space-y-10 sticky top-24">
      ...
      <div>
        <h3 className="text-lg font-semibold text-[#1d1d1f] mb-6 border-b border-[#d2d2d7] pb-2">
          Latest Posts
        </h3>
        <div className="space-y-6">
          {latestPosts.map((post) => {
            const isReview = "rating" in post;
            const link = isReview ? `/${post.slug}` : `/blog/${post.slug}`;
            return (
              <Link
                key={post.id}
                to={link}
              ...
              >
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-semibold text-[#1d1d1f] group-hover:text-[#2997ff] transition-colors line-clamp-2 mb-1">
                    {post.title}
                  </h4>
