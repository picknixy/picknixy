...
const searchString = "export default defineConfig(({mode}) => {";

if (content.includes("name: 'generate-sitemap'")) {
    console.log("Plugin already injected.");
} else {
    const pluginCode = `
const sitemapPlugin = () => {
  return {
...
`;
    ...
    content = content.replace(searchString, pluginCode + "\n" + searchString);
    content = content.replace("plugins: [react(), tailwindcss()],", "plugins: [react(), tailwindcss(), sitemapPlugin()],");
    fs.writeFileSync('vite.config.ts', content);
    console.log("Injected custom sitemap Vite plugin successfully.");
}
