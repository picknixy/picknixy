...
// We need to locate the cashscroller-review object and replace it.
// To do this safely, we will find the index of `{ id: "cashscroller-review",` and the next `},` that corresponds to its end, but since it has nested objects, it's safer to just rebuild it if we can find its boundaries.

const startIndex = mockData.indexOf('{    id: "cashscroller-review",');
// Assuming the review object ends before the export const categories or the end of the array.
// We added it at the end of the reviews array. Let's find `];` after the cashscroller review.
const endIndex = mockData.indexOf('];', startIndex);
...
if (startIndex !== -1 && endIndex !== -1) {
  const newReview = `{
    ...
    content: \`
      ...
      <p class="mb-4">Let's clear the air immediately: CashScroller is an innovative automation tool that was created to help users get more out of their time spent online by utilizing real-time engagement trends and AI-assisted content generation. It streamlines the process of publishing posts across digital platforms.</p>
...
`;
  ...
  // Since we don't know the exact formatting of the start, we'll slice and concatenate
  const newMockData = mockData.slice(0, startIndex) + newReview + '\n' + mockData.slice(endIndex);
  
  fs.writeFileSync(mockDataPath, newMockData, 'utf8');
  console.log('Successfully updated CashScroller review.');
} else {
  console.log('Could not find the target string bounds.', startIndex, endIndex);
}
